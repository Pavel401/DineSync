...
class _CustomQRPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final qrOffset =
        Offset((size.width - qrSize) / 2, (size.height - qrSize) / 2);
...
class CustomPrinterPainter extends CustomPainter {
  final List<TextInfo> textList;

  CustomPrinterPainter({
    required this.textList,
  });

  @override
  void paint(Canvas canvas, Size size) {
    // Draw a rectangle for visualizing the canvas boundaries (optional)
    final paint = Paint()..color = Colors.white.withOpacity(0.3);
    canvas.drawRect(Offset.zero & size, paint);
